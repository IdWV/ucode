/*
 * Copyright (C) 2025,2026 Isaac de Wolff <idewolff@gmx.com>
 *
...
static void
uc_promise_method_free( async_manager_t *manager, async_promise_t *promise, async_promise_method_t *func)
{
	if (!func)
		return;
...
static void async_promise_method_pushback( async_manager_t *manager, async_promise_t *promise, async_promise_method_t *func)
{
	...
	/* as long as the function is pending...
	counterpart is in uc_promise_method_free */
	promise->header.refcount++;
...
static const char *_strPromiseResolve = "async.promise.resolve";
static const char *_strPromiseReject = "async.promise.reject";
...
close_promise(void *ud)
{
	...
	if (promise)
	{
		DEBUG_PRINTF("%-1.3lf close promise %p %u\n", async_manager_uptime(promise->manager), promise, promise->manager->pending_promises_cnt);
...
async_promise_do( async_manager_t *manager, async_todo_t *todo)
{
	...
	for (; promise->stack;)
	{
		...
		uc_promise_method_free( manager, promise, next_to_be_handled);
	...
	{   // Is the result a promise?
		...
		if (ppromise && *ppromise)
		{
			...
			new_promise->header.promise_state = 0;
...
static void
async_promise_resolve_or_reject( async_manager_t *manager, async_promise_t *promise, uc_value_t *res, int type)
{
	if (promisePending != promise->header.promise_state)
	{
...
async_resolve_or_reject( async_manager_t *manager, async_promise_resolver_t *resolver, uc_value_t *res, int type)
{
	...
	async_promise_resolve_or_reject( manager, promise, res, type );
}
...
{
	async_promise_resolver_t **resolver = 0;
	if( type == promiseThen )
		resolver = (async_promise_resolver_t **)uc_fn_this(_strPromiseResolve);
	else if( type == promiseCatch )
		resolver = (async_promise_resolver_t **)uc_fn_this(_strPromiseReject);
...
static void
async_resolver_destroy( void *p )
{
	async_promise_resolver_t *resolver = (async_promise_resolver_t*)p;
	async_promise_resolver_unlink( 0, resolver );
}

static bool
async_promise_withresolvers( async_manager_t *manager, async_promise_resolver_t *resolver, uc_value_t **result )
{
	if( 0 == manager || 0 == manager->vm )
		return false;
	
	uc_vm_t *vm = manager->vm;
	async_promise_t *promise = 0;

	if( 0 == resolver )
	{
		promise = uc_promise_new( manager );
		resolver = async_promise_resolver_new(promise);
	}

	if( 0 == manager->promise_resolve_type )
	{
		static const uc_function_list_t fns[] = {
			{"__call__", async_resolver_resolve },
		};
		manager->promise_resolve_type = uc_type_declare( vm, _strPromiseResolve, fns, async_resolver_destroy );
	}

	if( 0 == manager->promise_reject_type )
	{
		static const uc_function_list_t fns[] = {
			{"__call__", async_resolver_reject },
		};
		manager->promise_reject_type = uc_type_declare( vm, _strPromiseReject, fns, async_resolver_destroy );
	}



	resolver->refcount++;
	uc_value_t *resolve = ucv_resource_new( manager->promise_resolve_type, resolver );
	resolver->refcount++;
	uc_value_t *reject = ucv_resource_new( manager->promise_reject_type, resolver );

	result[ 0 ] = promise ? uc_resource_new( manager->promise_type, promise ) : 0;
	result[ 1 ] = resolve;
	result[ 2 ] = reject;
...
uc_resolver_immediate( uc_vm_t *vm, void *user, int flags )
{
	...
	{
		uc_value_t *resolve_reject[ 3 ];
		if( !async_promise_withresolvers( manager, resolver, resolve_reject ) )
			return EXCEPTION_RUNTIME;

		uc_vm_stack_push(vm, ucv_get(resolver->callback));
		uc_vm_stack_push(vm, resolve_reject[ 1 ] );
		uc_vm_stack_push(vm, resolve_reject[ 2 ] );
...
_uc_async_new_promise( struct uc_async_manager *_man, void *arg, int type )
{
	switch( type )
	{
		case 0: // plain promise
		{
			uc_async_promise_resolver_t **resolver = arg;
			if( !resolver )
				return 0;
		...
		case 1: // promise.resolve()
		case 2: // promise.reject()
		{
			uc_value_t *result = arg;
			async_manager_t *manager = async_manager_cast( _man );
			async_promise_t *promise = uc_promise_new( manager );
			async_promise_resolve_or_reject( manager, promise, result, type == 1 ? promiseThen : promiseCatch );
			return uc_resource_new( manager->promise_type, promise);
		}
	}
	return 0;
...
/**
 * Creates and returns a fulfilled, resolved promise.
 *
 * @function module:async#Promise.resolve(result)
 *
  * @param {Any} result
 * Optional result passed to then().
 *
 * @returns {?module:async.promise}
 *
 * @example
...
static uc_value_t *
PromiseResolve(uc_vm_t *vm, size_t nargs)
{
	...
	async_promise_t *promise = uc_promise_new( manager );
	promise->header.refcount++;
	async_promise_resolve_or_reject( manager, promise, arg, promiseThen );
	
	return uc_resource_new( manager->promise_type, promise );
}

/**
 * Creates and returns a fulfilled, rejected promise.
 *
 * @function module:async#Promise.reject(result)
 *
  * @param {Any} error
 * Optional error passed to the catch().
 *
 * @returns {?module:async.promise}
 *
 * @example
...
static uc_value_t *
PromiseReject(uc_vm_t *vm, size_t nargs)
{
	uc_value_t *arg = uc_fn_arg(0);
	...
	async_promise_t *promise = uc_promise_new( manager );
	promise->header.refcount++;
	async_promise_resolve_or_reject( manager, promise, arg, promiseCatch );
 ...
 *
 * @function module:async#Promise.All
 *
 ...
 * 
 * async.Promise.All( promises ).then( (a)=>
 * {
 *	 print( 'fullfilled ', a, '\n' );
 * }).catch( (a)=>
 * {
 ...
 *
 * @function module:async#Promise.Any
 *
 * @param {Array} promises
 * Array of {@link module:async.promise|promises}. Elements in the array which are no promise are ignored.
 ...
 *				  NewPromise(400,'C',true) ];
 * 
 * async.Promise.Any( promises ).then( (a)=>
 * {
 *	 print( 'fullfilled ', a, '\n' );
 * }).catch( (a)=>
 * {
 ...
 * in the array.
 *
 * @function module:async#Promise.Race
 *
 * @param {Array} promises
 ...
 * 
 * async.Promise.Race( promises ).then( (a)=>
 * {
 *	 print( 'fullfilled ', a, '\n' );
 * }).catch( (a)=>
 * {
 ...
 *
 * @function module:async#Promise.AllSettled
 *
 * @param {Array} promises
 ...
 *				  NewPromise(50,'C',true) ];
 * 
 * async.Promise.AllSettled( promises ).then( (a)=>
 * {
 *	 print( 'fullfilled ', a, '\n' );
 * }).catch( (a)=>
 * {
...
/**
 * Returns an object containing a new promise, and the resolve and reject functions.
 * 
 * @function module:async#Promise.withResolvers
 *
 * @returns {Object}
 * 
 * @example
 * const result = async.Promise.withResolvers();
 * print( result );
 * // will output '{ "promise": "<async.promise ADDRESS>", "resolve": "<async.promise.resolve ADDRESS>", "reject": "<async.promise.reject ADDRESS>" }
 * 
 * // which is exactly equivalent of 
 * let result = {};
...
static uc_value_t *
PromisewithResolvers(uc_vm_t *vm, size_t nargs)
{
	async_manager_t *manager = async_manager_get( vm );
	uc_value_t *result[ 3 ];
	if( async_promise_withresolvers( manager, 0, result ) )
	{
		uc_value_t *obj = ucv_object_new( vm );
		ucv_object_add( obj, "promise", result[ 0 ] );
		ucv_object_add( obj, "resolve", result[ 1 ] );
		ucv_object_add( obj, "reject", result[ 2 ] );
		return obj;
	}
	return 0;
}
...
void async_promise_init( async_manager_t *manager, uc_value_t *scope )
{
	...
	uc_value_t *promise_proto = ucv_object_new( manager->vm );
	ucv_object_add( promise_proto, "__call__", ucv_cfunction_new( "Promise", Promise ) );
	ucv_object_add( promise_proto, "resolve", ucv_cfunction_new( "Promise.resolve", PromiseResolve ) );
	ucv_object_add( promise_proto, "reject", ucv_cfunction_new( "Promise.reject", PromiseReject ) );
	ucv_object_add( promise_proto, "All", ucv_cfunction_new( "Promise.All", PromiseAll ) );
	ucv_object_add( promise_proto, "Any", ucv_cfunction_new( "Promise.Any", PromiseAny ) );
	ucv_object_add( promise_proto, "Race", ucv_cfunction_new( "Promise.Race", PromiseRace ) );
	ucv_object_add( promise_proto, "AllSettled", ucv_cfunction_new( "Promise.AllSettled", PromiseAllSettled ) );
	ucv_object_add( promise_proto, "withResolvers", ucv_cfunction_new( "Promise.withResolvers", PromisewithResolvers ) );

	uc_value_t *promise = ucv_object_new( manager->vm );
	ucv_prototype_set( promise, promise_proto );


	ucv_object_add( scope, "Promise", promise );
	ucv_object_add( scope, "throw", ucv_cfunction_new( "throw", Throw ) );
}

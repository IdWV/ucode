...
{
    if( !vm || !vm->async_manager )
        return 0;
    return (async_manager_t *)vm->async_manager;
}
...
uptime(uc_vm_t *vm)
{
    async_manager_t *handler = async_manager_get( vm );
    if (!handler)
        return NAN;
    int64_t now = async_timer_current_time() - handler->start_time;
...
async_unique_in_vm_new(uc_vm_t *vm)
{
	...
	uc_vm_stack_push(vm, ucv_uint64_new( SIGNEWCALLBACK ));
	uc_vm_stack_push(vm, func);

	if (ucsignal(vm, 2) != func)
...
static uc_value_t *
async_callback_get_ucode_func( uc_vm_t *vm, struct async_callback *cb )
{
...
static int
async_callback_call(uc_vm_t *vm, struct async_callback *cb, uc_value_t **args, size_t nargs, uc_value_t **ret, bool cleanup)
{
    ...
    case callbackC_int_user_flags:
    {
        if (!cb->c_int_user_flags.func)
            return EXCEPTION_NONE;
        int ex = (*cb->c_int_user_flags.func)(vm, cb->c_int_user_flags.user, flags);
        if (cleanup)
        {
    ...
    case callbackC_int_user_args_flags:
    {
        ...
        for (size_t n = 0; n < cb->nargs; n++)
            args2[m++] = cb->args[n];
        int ex = (*cb->c_int_user_args_flags.func)(vm, cb->c_int_user_args_flags.user, args2, m, flags);
        if (cleanup)
        {
...
uc_timer_ucode_new(uc_vm_t *vm, size_t nargs, uc_value_t *cb, size_t startarg)
{
...
static async_timer_t *
async_timer_c_int_user_flags_new(uc_vm_t *vm, int (*func)(uc_vm_t *, void *, int), void *user)
{
...
static int
async_promise_destroy(uc_vm_t *, async_todo_t *);

static int
async_todo_unlink(uc_vm_t *vm, async_todo_t *todo)
{
    if (!todo)
        return EXCEPTION_NONE;
    ...
    case todoTimer:
        async_callback_destroy(vm, &async_timer_cast(todo)->callback);
        break;
    case todoPromise:
        ret = async_promise_destroy(vm, todo);
        break;
    }
...
static void
async_todo_put_in_list(uc_vm_t *vm, async_todo_t *todo)
{
    async_manager_t *phandler = async_manager_get( vm );
    ...
    if( due )
    {
        DEBUG_PRINTF( "%-1.3lf Todo %p scheduled at %-1.3lf\n", uptime(vm), todo, 
            (due - phandler->start_time) / 1000.0 );

    }
    async_todo_t *previous = 0;
    for (async_todo_t *it = phandler->todo_list; it; it = it->next)
    {
...
static int
async_promise_do(uc_vm_t *, async_todo_t *);

static int
async_handle_todo(uc_vm_t *vm)
{
    async_manager_t *phandler = async_manager_get( vm );
    if (!phandler->todo_list)
        return EXCEPTION_NONE;
    int64_t now = async_timer_current_time();
    async_todo_t *to_be_handled = 0;

    while (phandler->todo_list)
    {
        bool end_of_todo_for_now = false;
        switch ((enum async_todo_type)phandler->todo_list->todo_type)
        {
        case todoClearedTimer:
            break;
        case todoTimer:
            end_of_todo_for_now = async_timer_cast(phandler->todo_list)->due > now;
            break;
        case todoPromise:
            break;
        }
        ...
        async_todo_t *pop = phandler->todo_list;
        phandler->todo_list = pop->next;
        pop->in_todo_list = 0;
        pop->next = 0;

        if (todoClearedTimer == pop->todo_type)
        {
            async_todo_unlink(vm, pop);
        }
        else
        {
    ...
    while (to_be_handled)
    {
        ...
        case todoTimer:
        {
            ...
            ex = async_callback_call(vm, &timer->callback, 0, 0, 0, false);
            if (0 == timer->periodic ||
                // the timer can be cleared in the callback itself
                todoClearedTimer == timer->header.todo_type)
                break;
            timer->due += timer->periodic;
            async_todo_put_in_list(vm, &timer->header);
            break;
        }
        case todoPromise:
        {
            ex = async_promise_do(vm, pop);
            break;
        }
        }

        {
            int ex2 = async_todo_unlink(vm, pop);
        ...
        if (EXCEPTION_NONE != ex)
        {
            ...
            while (to_be_handled)
            {
                async_todo_t *pop = to_be_handled;
                to_be_handled = pop->next;
                pop->next = phandler->todo_list;
                phandler->todo_list = pop;
                pop->in_todo_list = 1;
            }
            return ex;
        }
...
static int64_t
async_how_long_to_next_todo(uc_vm_t *vm)
{
    async_manager_t *phandler = async_manager_get( vm );
    while (phandler->todo_list)
    {
        ...
        {
            int64_t now = async_timer_current_time();
            int64_t due = async_timer_cast(phandler->todo_list)->due;
            if (due > now)
                return due - now;
            return 0;
        }
...
createTimer(uc_vm_t *vm, size_t nargs, int type)
{
    ...
    async_timer_t *timer = uc_timer_ucode_new(vm, nargs, cb, startarg);
    ...
    async_manager_t *phandler = async_manager_get( vm );
    if (0 == phandler->timer_type)
    {
        static const uc_function_list_t timer_fns[] = {};
        phandler->timer_type = uc_type_declare(vm, _strTimer, timer_fns, close_timer);
    }

    timer->header.refcount++;
    return uc_resource_new(phandler->timer_type, timer);
}
...
static void
uc_promise_func_free(uc_vm_t *vm, async_promise_method_t *func)
{
    if (!func)
        return;
    async_callback_destroy(vm, &func->callback);
    free(func);
}

static void
async_exception_clear(uc_vm_t *vm, uc_exception_t *exception)
{
...
static void
async_exception_free(uc_vm_t *vm, uc_exception_t *exception)
{
    if (!exception)
        return;
    async_exception_clear(vm, exception);
    free(exception);
}

static void
async_exception_move(uc_vm_t *vm, uc_exception_t *to, uc_exception_t *from)
{
    if (from && to)
    {
        to->type = from->type;
...
async_exception_new(uc_vm_t *vm, uc_exception_t *exception)
{
    uc_exception_t *ret = xalloc(sizeof(uc_exception_t));
    if (exception)
    {
        async_exception_move(vm, ret, exception);
    }
    return ret;
}
...
uc_promise_new(uc_vm_t *vm)
{
    ...
    p->vm = vm;
    async_manager_t *phandler = async_manager_get( vm );
    phandler->pending_promises_cnt++;
    p->header.promise_pending = 1;

    DEBUG_PRINTF("%-1.3lf new promise %p %u\n", uptime(vm), p, phandler->pending_promises_cnt);
    return p;
}
...
static void
async_promise_clear_result( uc_vm_t *vm, async_promise_t *promise )
{
    if( !promise )
        return;
    ...
    {
        async_exception_free(vm, promise->result.exception);
...
static int
async_promise_destroy(uc_vm_t *vm, async_todo_t *todo)
{
    async_promise_t *promise = async_promise_cast(todo);

    async_manager_t *vm_is_active = async_manager_get( vm );
    ...
    if (uncaught)
    {
        if (vm_is_active && promise->header.promise_result_is_exception)
        {
            ...
            async_exception_clear(vm, &vm->exception);
            async_exception_move(vm, &vm->exception, promise->result.exception);
            async_exception_free(vm, promise->result.exception);
            promise->result.exception = 0;
            promise->header.promise_result_is_exception = 0;
    ...
    async_promise_clear_result( vm, promise );
    ...
    for (; stack;)
    {
        ...
        if (vm_is_active && !(uncaught) && promiseFinally == pop->callback.type)
            async_callback_call(vm, &pop->callback, 0, 0, 0, false);
...
#pragma GCC diagnostic ignored "-Wuse-after-free"
        ...
        if (vm_is_active)
        {
            ...
            {
                // TODO: Somehow setup a stackframe to let the exception handler
                // point to caused_by as culprit
                pop = true;
//               	uc_vm_stack_push(vm, ucv_get(caused_by) );
//                uc_vm_stack_push(vm, ucv_boolean_new(true) );
            }
            uc_vm_raise_exception(vm, ret = EXCEPTION_RUNTIME, err, promise);
            if( pop )
            {
//                ucv_put( uc_vm_stack_pop( vm ) );
//               ucv_put( uc_vm_stack_pop( vm ) );
...
static void async_promise_method_pushback(uc_vm_t *vm, async_promise_t *promise, async_promise_method_t *func)
{
    ...
    if (0 == promise->header.promise_pending)
    {
        ...
        async_manager_get( vm )->pending_promises_cnt++;
    }

    if ((0 == promise->header.in_todo_list) && 
        (promisePending != promise->header.promise_state))
    {
        async_todo_put_in_list(vm, &promise->header);
    }
}

static void
async_promise_add_ucode_func(uc_vm_t *vm, async_promise_t *promise, uc_value_t *func, int type)
{
    ...
    async_promise_method_pushback(vm, promise, pf);
}

static void
async_promise_add_c_int_user_args_flags_func(uc_vm_t *vm, async_promise_t *promise,
...
async_promise_then(uc_vm_t *vm, size_t nargs)
{
    ...
    if (!ppromise || !*ppromise)
        return 0;
    async_promise_add_ucode_func(vm, *ppromise, uc_fn_arg(0), promiseThen);
...
async_promise_catch(uc_vm_t *vm, size_t nargs)
{
    ...
    if (!ppromise || !*ppromise)
        return 0;
    async_promise_add_ucode_func(vm, *ppromise, uc_fn_arg(0), promiseCatch);
    return ucv_get(_uc_fn_this_res(vm));
}
...
async_promise_finally(uc_vm_t *vm, size_t nargs)
{
    ...
    if (!ppromise || !*ppromise)
        return 0;
    async_promise_add_ucode_func(vm, *ppromise, uc_fn_arg(0), promiseFinally);
    return ucv_get(_uc_fn_this_res(vm));
}
...
close_promise(void *ud)
{
    ...
    if (promise)
    {
        DEBUG_PRINTF("%-1.3lf close promise %p %u\n", uptime(promise->vm), promise, promise->header.refcount);
        async_todo_unlink(promise->vm, &promise->header);
    }
}

static int
async_promise_do(uc_vm_t *vm, async_todo_t *todo)
{
    async_promise_t *promise = async_promise_cast(todo);
    async_manager_t *handler = async_manager_get( vm );
    ...
    for (; promise->stack;)
    {
        ...
        uc_promise_func_free(vm, next_to_be_handled);
    ...
    if( !next_to_be_handled )
    {
        ...
        {
            promise->header.promise_pending = 0;
            if( handler ) handler->pending_promises_cnt--;
        }
        return EXCEPTION_NONE;
    }
    ...
    int ex = EXCEPTION_NONE;
    {
        ...
        {
            async_promise_t *push = handler->active_promise;
            handler->active_promise = promise;
            ex = async_callback_call(vm, &next_to_be_handled->callback, &in, 1, &out, true);
            handler->active_promise = push;
        }

        ucv_put(in);
    }

    uc_value_t *caused_by = async_callback_get_ucode_func( vm, &next_to_be_handled->callback );
    ...
    if (EXCEPTION_NONE != ex)
    {
        ...
        else 
        {
            ...
            promise->result.exception = async_exception_new(vm, &vm->exception);
...
static void
async_resolve_or_reject(uc_vm_t *vm, async_promise_resolver_t *resolver, uc_value_t *res, int type)
{
    ...
    async_todo_unlink(vm, &promise->header);
}
...
async_resolver_resolve_or_reject(uc_vm_t *vm, size_t nargs,int type)
{
    ...
    async_resolve_or_reject(vm, *presolve, uc_fn_arg(0), type);
    return 0;
}
...
static int 
async_promise_resolver_unlink( uc_vm_t *, async_promise_resolver_t *resolver )
{
...
uc_resolver_immediate(uc_vm_t *vm, void *user, int flags)
{
    ...
    if (flags & UC_ASYNC_CALLBACK_FLAG_EXECUTE)
    {
        ...
        resolver->refcount++;
        uc_vm_stack_push(vm, uc_resource_new(async_manager_get( vm )->resolver_type, resolver));
    ...
    {
        int ex2 = async_promise_resolver_unlink( vm, resolver );
        if( EXCEPTION_NONE == ex )
            ex = ex2;
    }
    return ex;
...
static int async_promise_array_resolve(uc_vm_t *vm, struct async_promise_array *promise_array, uc_value_t **args, size_t nargs, int type)
{
    ...
    else 
    {
        ...
        switch( value_type )
        {
            ...
            {
                value = ucv_array_new_length( vm, promise_array->numresults );
                for( uint32_t n=0; n<promise_array->numresults; n++ )
                {
        ...
        async_resolve_or_reject(vm, promise_array->resolver, value, type);
    }
    async_promise_resolver_unlink( vm, promise_array->resolver);
...
static int async_promise_array_immediate(uc_vm_t *vm, void *user, int flags)
{
    ...
    if (flags & UC_ASYNC_CALLBACK_FLAG_EXECUTE)
    {
        ...
        ex = async_promise_array_resolve(vm, promise_array, 0, 0, type);
    }
    if (flags & UC_ASYNC_CALLBACK_FLAG_CLEANUP)
...
static int async_promise_array_then(uc_vm_t *vm, void *user, uc_value_t **args, size_t nargs, int flags)
{
    ...
    if (flags & UC_ASYNC_CALLBACK_FLAG_EXECUTE)
    {
        ...
        if( 0 == cnt )
            ex = async_promise_array_resolve(vm, promise_array, args, nargs, promiseThen);
    }
    if (flags & UC_ASYNC_CALLBACK_FLAG_CLEANUP)
    {
        async_promise_array_unlink(promise_array);
...
static int async_promise_array_catch(uc_vm_t *vm, void *user, uc_value_t **args, size_t nargs, int flags)
{
    ...
    if (flags & UC_ASYNC_CALLBACK_FLAG_EXECUTE)
    {
        ...
        if( 0 == cnt )
            ex = async_promise_array_resolve(vm, promise_array, args, nargs, promiseCatch);
    }
    if (flags & UC_ASYNC_CALLBACK_FLAG_CLEANUP)
    {
        async_promise_array_unlink(promise_array);
...
async_promise_array_new( uc_vm_t *vm, size_t nargs, int type )
{
    ...
    async_promise_t *promise = uc_promise_new(vm);
    ...
    for (size_t n = 0; n < length; n++)
    {
        ...
        if (ppromise && *ppromise)
        {
            ...
            async_promise_add_c_int_user_args_flags_func(vm, *ppromise, async_promise_array_then, slot, promiseThen);
            promise_array->refcount++;
            async_promise_add_c_int_user_args_flags_func(vm, *ppromise, async_promise_array_catch, slot, promiseCatch);
        }
    ...
    if (0 == promise_array->exec_refcount && 0 != promise_array->resolver->promise )
    {
        ...
        promise_array->refcount++;
        async_timer_t *timer = async_timer_c_int_user_flags_new(vm, async_promise_array_immediate, promise_array);
        async_todo_put_in_list(vm, &timer->header);
        timer->header.refcount--;
    }

    return uc_resource_new(async_manager_get( vm )->promise_type, promise);
}

static struct uc_value *
_uc_async_new_promise(uc_vm_t *vm, uc_async_promise_resolver_t **resolver)
{
    if( !resolver )
        return 0;
    async_promise_t *promise = uc_promise_new(vm);
...
static void
_uc_async_resolve_reject(uc_vm_t *vm, uc_async_promise_resolver_t **resolver, uc_value_t *res, bool resolve)
{
    if (!resolver || !*resolver)
        return;
    async_promise_resolver_t *res2 = async_promise_resolver_cast(*resolver);
    *resolver = 0;
    async_resolve_or_reject(vm, res2, res, resolve ? promiseThen : promiseCatch);
    async_promise_resolver_unlink( vm, res2);
}
...
static int
async_handle_callbacks(uc_vm_t *vm)
{
    async_manager_t *phandler = async_manager_get( vm );
    if( 0 == phandler || 0 == phandler->callback_queuer )
        return EXCEPTION_NONE;
    async_callback_queuer_t *pchandler = async_callback_queuer_cast(phandler->callback_queuer);
    struct async_callback_link *stack = uc_unique_lock_stack(pchandler->unique_in_vm);
    uc_unique_unlock_stack(pchandler->unique_in_vm, 0);
    ...
    while (stack)
    {
        ...
        stack = pop->next;
        int ex = async_callback_call(vm, &pop->callback, 0, 0, 0, true);
        free( pop );
        if (EXCEPTION_NONE == ex)
            continue;
        if (stack)
        {
            ...
            reverse_stack(struct async_callback_link, stack);
            last->next = uc_unique_lock_stack(pchandler->unique_in_vm);
            uc_unique_unlock_stack(pchandler->unique_in_vm, stack);
        }
        return ex;
    }
    return EXCEPTION_NONE;
...
static bool
async_any_callbacks_waiting(uc_vm_t *vm)
{
    async_manager_t *phandler = async_manager_get( vm );
    if (0 == phandler || 0 == phandler->callback_queuer)
        return false;
    async_callback_queuer_t *pchandler = async_callback_queuer_cast(phandler->callback_queuer);
    if ((intptr_t)pchandler->unique_in_vm->stack & ~(intptr_t)3)
        return true;
    return false;
}

static bool
_uc_async_request_callback(struct uc_async_callback_queuer const *phandler,
                           int (*func)(struct uc_vm *, void *, int), void *user)
{
    ...
    const async_callback_queuer_t *pchandler = async_callback_queuer_cast_const(phandler);

    struct async_callback_link *stack = uc_unique_lock_stack(pchandler->unique_in_vm);

    if (stack == (struct async_callback_link *)pchandler->unique_in_vm)
    {
    ...
    pfunc->next = stack;
    uc_unique_unlock_stack(pchandler->unique_in_vm, pfunc);

    async_unique_wakeup( pchandler->unique_in_vm );
    return true;
}

static void
_uc_async_callback_queuer_free(struct uc_async_callback_queuer const **pphandler)
{
    if (0 == pphandler || 0 == *pphandler)
        return;
    async_callback_queuer_t const *pchandler = async_callback_queuer_cast_const(*pphandler);
    *pphandler = 0;

    struct async_callback_unique_in_vm *unique_in_vm = pchandler->unique_in_vm;
    free((void *)pchandler);
...
static int _async_create_timer(uc_vm_t *vm, void *user, int flags)
{
    ...
    if (flags & UC_ASYNC_CALLBACK_FLAG_CLEANUP)
    {
        async_todo_unlink(vm, &timer->header);
    }
    return EXCEPTION_NONE;
}

static uc_async_timer_t *
_uc_async_create_timer(struct uc_async_callback_queuer const *handler,
                       int (*cb)(uc_vm_t *, void *, int), void *user, uint32_t msec, bool periodic)
{
    ...
    async_callback_queuer_t const *pchandler = async_callback_queuer_cast_const(handler);

    // are we synchron?
    uc_vm_t *vm = async_unique_is_synchron(pchandler->unique_in_vm);
    if (vm)
    {
        _async_create_timer(vm, timer, UC_ASYNC_CALLBACK_FLAG_EXECUTE | UC_ASYNC_CALLBACK_FLAG_CLEANUP);
    }
    else
    {
        _uc_async_request_callback(handler, _async_create_timer, timer);
    }
...
static void
_uc_async_free_timer(struct uc_async_callback_queuer const *handler,
                     uc_async_timer_t **_pptimer, bool clear)
{
    ...
    *_pptimer = 0;
    ...
    if (vm)
    {
        _async_free_timer(vm, timer, UC_ASYNC_CALLBACK_FLAG_EXECUTE | UC_ASYNC_CALLBACK_FLAG_CLEANUP);
    }
    else
    {
        _uc_async_request_callback(handler, _async_free_timer, timer);
    }
}
...
_uc_async_new_callback_queuer(struct uc_vm *vm)
{
    async_manager_t *phandler = async_manager_get( vm );
    if( 0 == phandler )
        return 0;

    if (0 == phandler->callback_queuer)
    {
        async_callback_queuer_t *pcallbackhandler = async_callback_queuer_new();
        phandler->callback_queuer = &pcallbackhandler->header;
        pcallbackhandler->unique_in_vm = async_unique_in_vm_new(vm);
    }
...
static void
async_callback_free(uc_vm_t *vm, struct uc_async_callback_queuer *phandler)
{
    if (0 == phandler)
        return;
    async_callback_queuer_t *pchandler = async_callback_queuer_cast(phandler);
    struct async_callback_link *stack = uc_unique_lock_stack(pchandler->unique_in_vm);
    // write sentinel value meaning that callbacks are disabled forever
    uc_unique_unlock_stack(pchandler->unique_in_vm, (void *)pchandler->unique_in_vm);

    struct uc_async_callback_queuer const *pconsth = phandler;
...
static void
async_manager_free(uc_vm_t *vm, async_manager_t *phandler)
{
    while (phandler->todo_list)
    {
        async_todo_t *pop = phandler->todo_list;
    ...
    async_callback_free(vm, phandler->callback_queuer);
    free(phandler);
}

static int
async_event_pump(uc_vm_t *vm, unsigned max_wait, int flags)
{
    if (flags & UC_ASYNC_PUMP_PUMP)
    {
        ...
        async_manager_t *phandler =
            async_manager_get( vm );

        do
        {
            DEBUG_PRINTF("%-1.3lf Pump!\n", uptime(vm));
            int ex = async_handle_todo(vm);
            if (EXCEPTION_NONE != ex)
            {
                if (EXCEPTION_EXIT == ex)
                    return STATUS_EXIT;
            ...
            int64_t tosleep = async_how_long_to_next_todo(vm);

            if (-1 == tosleep) // no todo list
            {
                if( 0 == phandler->pending_promises_cnt ) // no pending promises
                {
            ...
            if (max_wait && !async_any_callbacks_waiting(vm))
            {
                ...
                if (tosleep > 0)
                {
                    ...
                    if( !async_any_callbacks_waiting(vm ) )
    ...
    if (flags & UC_ASYNC_PUMP_CLEANUP)
    {
        async_manager_t *phandler = async_manager_get( vm );
        vm->async_manager = 0;
        async_manager_free(vm, phandler);
    }
...
Promise(uc_vm_t *vm, size_t nargs)
{
    ...
    async_timer_t *timer = async_timer_c_int_user_flags_new(vm, uc_resolver_immediate, resolver);
    async_todo_put_in_list(vm, &timer->header);
    timer->header.refcount--;
...
{
    async_manager_t *handler = async_manager_get( vm );
    async_promise_t *promise = (0 == handler) ? 0 : handler->active_promise;
...
PumpEvents(uc_vm_t *vm, size_t nargs)
{
    ...
    async_event_pump(vm, msec, flags);

    async_manager_t *handler = async_manager_get( vm );
    if (handler)
    {
        if (handler->pending_promises_cnt ||
            handler->todo_list)
            return ucv_boolean_new(true);
    }
    return ucv_boolean_new(false);
...
{
    return ucv_double_new(uptime(vm));
}
#endif
...
void uc_module_init(uc_vm_t *vm, uc_value_t *scope)
{
    ...
    vm->async_manager = xalloc(sizeof(async_manager_t));
    vm->async_manager->event_pump = async_event_pump;
    vm->async_manager->new_promise = _uc_async_new_promise;
    vm->async_manager->resolve_reject = _uc_async_resolve_reject;
    vm->async_manager->new_callback_queuer = _uc_async_new_callback_queuer;

    async_manager_t *handler = async_manager_get( vm );

    /* promise initializing */
    handler->promise_type = uc_type_declare(vm, _strPromise, promise_type_fns, close_promise);
    handler->resolver_type = uc_type_declare(vm, _strResolver, resolver_type_fns, close_resolver);

    uc_function_list_register(scope, local_async_fns);

#ifdef HAS_UPTIME
    handler->start_time = async_timer_current_time();
#endif
}

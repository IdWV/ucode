 ...
 * pointer stored in the uc_vm_t object, which is initialized in the
...
struct uc_async_manager
{
    ...
    // promise functions
    struct uc_value *(*new_promise)( struct uc_async_manager *, void *, int );
    ...
    // alien call function
    uc_async_alien_t const *(*new_alien)( struct uc_async_manager * );
};
 ...
 * When 'flags' has UC_ASYNC_CALLBACK_FLAG_EXECUTE set, the callback should be executed,
 * when UC_ASYNC_CALLBACK_FLAG_CLEANUP is set, you should free up resources in *user.
 * When only UC_ASYNC_CALLBACK_FLAG_CLEANUP is set, 
 * it is possible the script is no longer running, and you shouldn't access vm.
 * 
 * The return value of (*cb)(...) should be EXCEPTION_NONE, unless you want an exception
*/
static inline bool 
 ...
 * and the resolver pointer will have a resolver handle.
 * 
 * This function has to be called from within the script thread.
*/
...
uc_async_promise_new( uc_vm_t *vm, uc_async_promise_resolver_t **resolver )
{
    ...
    if( !manager )
        return 0;
    return (*manager->new_promise)( manager, (void *)resolver, 0 );
}

/**
 * Create a new resolved promise. The return value is the promise.
 * 
 * This function has to be called from within the script thread.
*/
static inline uc_value_t *
uc_async_promise_new_resolve( uc_vm_t *vm, uc_value_t *result )
{
    struct uc_async_manager *manager = uc_async_manager_get( vm );
    if( !manager )
        return 0;
    return (*manager->new_promise)( manager, (void *)result, 1 );
}

/**
 * Create a new rejected promise. The return value is the promise.
 * 
 * This function has to be called from within the script thread.
*/
static inline uc_value_t *
uc_async_promise_new_reject( uc_vm_t *vm, uc_value_t *error )
{
    struct uc_async_manager *manager = uc_async_manager_get( vm );
    if( !manager )
        return 0;
    return (*manager->new_promise)( manager, (void *)error, 2 );
}
 ...
 * 
 * If the function returns NULL, either async is not imported, or async is compiled 
 * without alien support.
 */
static inline const uc_async_alien_t *
 ...
 */
static inline void
uc_async_alien_free( const uc_async_alien_t **alien )
{
    if( !alien || !*alien )
        return;
    (*(*alien)->free)( alien );
 ...
 */
static inline int 
uc_async_alien_call( const uc_async_alien_t *alien, int (*func)( uc_vm_t *, void *, int flags ), void *user )
{
    if( !alien )
        return INT_MIN;
